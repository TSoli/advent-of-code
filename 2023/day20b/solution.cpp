...
void propogateOutputs(
    ...
    std::unordered_map<std::string, int> &wasHigh) {
      ...
      if (flipFlops.contains(module)) {
        ...
        for (const auto &input : conjunctions[module].currInputs) {
          ...
          propogateOutputs(signal, module, conjunctions[module].outputs,
                           flipFlops, conjunctions, nextProcess, wasHigh);
        }
      }
    }
...
int main(int argc, char *argv[]) {
  ...
  // tested and the initial offset is the same as the loops after
  long pushes{0};
  auto loops{conjunctions["tj"].state};
  auto wasHigh{conjunctions["tj"].state};
  while (true) {
    pushButton(broadcaster, flipFlops, conjunctions, wasHigh);
    pushes++;
    for (auto &[k, v] : wasHigh) {
      if (v && !loops[k]) {
        loops[k] = pushes;
        wasHigh[k] = 0;
      }
  ...
  long totalPushes{1};
  for (const auto &[k, v] : loops) {
    totalPushes = std::lcm(totalPushes, v);
  }

  std::cout << "Took: " << totalPushes << " button pushes\n";
  return 0;
}
